...
These files are excluded from code2prompt because they are vendored libraries, model weights, WASM support files, and generated manifests. Their exact source URLs and refresh rules are documented in `lab-js/vendor/README.md` in the repository, but only this summary and the nested folder description are uploaded to the chatbot.
